...
class EarthsAtmosphere(with_metaclass(ABCMeta)):
    ...
    def calculate_density_spline(self, n_steps=2000):
        ...
        #self._s_h2X = UnivariateSpline(h_intp, np.log(X_intp), k=2, s=10.0)
        #self._s_X2rho = UnivariateSpline(X_int, vec_rho_l(dl_vec), k=2, s=10.0)
        #self._s_lX2h = UnivariateSpline(np.log(X_intp)[::-1], h_intp[::-1], k=2, s=10.0)
